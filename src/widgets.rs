...
use crossterm::{cursor, event::{self, Event, MouseButton, MouseEventKind}, execute, queue, style::{self, Stylize}, terminal, Command};
use mintymacks::{arrays::ArrayBoard, bits::BoardMask, model::{self, BoardFile, BoardRank, ChessPiece, Color, ColoredChessPiece, Square}};
...
impl BoardRenderer {
    pub const DARK: style::Color = style::Color::Rgb { r: 0x77, g: 0x66, b: 0x55 };
    pub const LIGHT: style::Color = style::Color::Rgb { r: 0xCC, g: 0xBB, b: 0xAA };
    pub const HIGHLIGHT: style::Color = style::Color::Rgb { r: 0xDD, g: 0x88, b: 0x88 };
    pub const GREY: style::Color = style::Color::Rgb { r: 0x88, g: 0x88, b: 0x88 };
    ...
    pub fn translate(&self, row: u16, col: u16) -> Option<Square> {
        ...
        let sq = Square::at(BoardFile::new(row_ix as i8).unwrap(),
    ...
    pub fn render(&self, board: &ArrayBoard<Option<ColoredChessPiece>>,
        highlight: BoardMask, selected: BoardMask) -> Vec<u8> {
        let mut res = vec![];
    ...
    pub fn square(&self, sq: Square, pc: Option<ColoredChessPiece>,
        highlight: bool, selected: bool, res: &mut Vec<u8>) {

        let (mut fg, mut bg) =
        if sq.bit() & 0x55AA55AA55AA55AA != 0 {
        ...
        let lsq = if self.rotated {
            Self::rotate(sq)
        } else {
            sq
        };
        let line1 = if sq.file_rank().0 == BoardFile::H {
            format!("    {}", lsq.file_rank().1.digit()).stylize().with(fg).on(bg)
        } else {
            format!("     ").with(fg).on(bg)
        };

        let line3 = if sq.file_rank().1 == BoardRank::_1 {
            format!("{}    ", lsq.file_rank().0.letter()).stylize().with(fg).on(bg)
        } else {
            format!("     ").with(fg).on(bg)
        };
        ...
        let line2 = if let Some(pc) = pc {
            (if selected {
                ...
                format!("  {}  ",  Self::piece(pc.piece()))
            }).stylize().with(Self::color(pc.color())).on(bg)
        } else if selected {
            format!("  \u{25CB}  ")
            .stylize().with(Self::GREY).on(bg)
        } else {
            format!("     ")
            .stylize().with(Self::GREY).on(bg)
        };
        ...
        queue!(
            ...
            cursor::MoveDown(1), cursor::MoveLeft(5),
    ...
    pub fn color(c: Color) -> style::Color {
        match c {
            Color::White => style::Color::Rgb { r: 0xFF, g: 0xFF, b: 0xFF },
